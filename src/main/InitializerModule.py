...
class InitializerModule(Module):
    ...
    **Configuration**:
    ...
    def run(self):
        ...
        device_whitelist = self.config.get_list("device_whitelist", None)
        use_experimental_features = self.config.get_bool("use_experimental_features", False)
        Initializer.init(horizon_color, compute_device, compute_device_type, device_whitelist, use_experimental_features, clean_up_scene=False)